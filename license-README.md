...
`yarn licenses list --production --recursive --json > results.json`
	...
	Last generated yarn license in [Legal](https://example.com/legal) e.g. yarnmodernlicenses-v30062025.json
...
3. Send the file to the legal colleagues and request a new deep license scan. The Legal Team provides a report detailing dependencies with incompatible licenses, along with suggestions for removal or updates.
	...
	Last deep license scan "OSS Inventory" in [Legal](https://example.com/legal) e.g. OSS-Inventory-CG_wikijs_20250701.xlsx
  
 4. Review & Apply in code the needed changes and repeat the process till all dependencies have compatible license.
  
> Info about which licenses are compatible can be found in the last deep license scan output in [Legal](https://example.com/legal). LegalEvaluationProject.xls contains a summary.
